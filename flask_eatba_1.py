...
from linebot.v3.messaging import (
    ...
    TemplateMessage,  # 用於模板消息
    QuickReply, QuickReplyItem,  # 快速回覆
    CarouselTemplate, CarouselColumn,  # 輪播模板
    ButtonsTemplate,  # 按鈕模板
    PostbackAction, MessageAction  # 行為動作
)
...
def handle_message(event):
    ...
    if "sample" in user_message:
        ...
        responses = [TextMessage(text="請輸入關鍵字 '美食推薦' 來順利操作小幫手~")] 
...
def handle_rests_recommand(user_id, section_name):
    def create_rest_col(rest_text, rest_title, rest_comment="",rest_address="",rest_phone="",rest_url=""):
        address = rest_address if rest_address else '這是地址'
        phone = rest_phone if rest_phone else '這是電話'
        ...
        return CarouselColumn(
            ...
            actions=[
                PostbackAction(label='餐廳地址', data=f'action=address&info={address}', display_text=f'餐廳地址: {address}'),
                PostbackAction(label='連絡電話', data=f'action=phon&info={phone}', display_text=f'連絡電話: {phone}'),
                PostbackAction(label='餐廳評價', data=f'action=comment&info={comment}', display_text=f'餐廳評價: {comment}')
            ]
        )

    def handle_postback(event):
        data = event.postback.data
        action, info = data.split('&info=')

        if action == 'action=address':
            reply_text = f"餐廳地址: {info}"
        elif action == 'action=phon':
            reply_text = f"連絡電話: {info}"
        elif action == 'action=comment':
            reply_text = f"餐廳評價: {info}"
